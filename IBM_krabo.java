	...
	private double distance(Instance first, Instance second)
	{
		...
		for(int i = 0; i < m_Train.numAttributes(); i++)
		{
			...
			if (m_Train.attribute(i).isNominal()) 
			{
				...
				//for(Instance currentInstance : m_Train)
				//for (int j=0; j < m_Train.numAttributes(); j++)
				for(Instance currentInstance : training_Items)
				{
					//Instances instances = m_Train.m_Instances;
					//Instance currentInstance = instances.get(j);
					//if (currentInstance.toString().equals(second.toString())) {
					if (currentInstance.equalHeaders(second) || currentInstance.equals(second)) {
